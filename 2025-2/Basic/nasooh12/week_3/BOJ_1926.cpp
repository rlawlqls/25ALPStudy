...
#define X first
#define Y second 
int board[502][502]; // 1 : 색칠 된 칸 / 0 : 색칠 안 된 칸
...
int main (void){
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < m; j++) { // (i, j)를 시작점으로 하고 싶은 상황
            if(board[i][j] == 0 || vis[i][j]) continue; // 해당 칸이 색칠 안된 칸이거나, 이미 방문했을 경우 넘어감
            ...
            while(!Q.empty()){
                ...
                for(int dir = 0; dir < 4; dir++){ // 상하좌우 칸을 살펴볼 것이다.
                    ...
                    if(vis[nx][ny] || board[nx][ny] != 1) continue; // 이미 방문한 칸이거나 색칠 된 칸이 아닐 경우
